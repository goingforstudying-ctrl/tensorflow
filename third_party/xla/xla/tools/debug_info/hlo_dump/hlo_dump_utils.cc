...
#include "xla/tools/debug_info/hlo_dump/hlo_dump_utils.h"
...
#include "third_party/gloop/base/file_toc.h"
#include "third_party/gloop/util/endian/endian.h"
#include "third_party/gloop/util/gtl/flat_set.h"
#include "re2/re2.h"
#include "riegeli/bytes/string_writer.h"
#include "riegeli/zlib/zlib_writer.h"
#include "xla/shape_util.h"
#include "xla/tools/debug_info/hlo_dump/hlo_dump_assets.h"
...
std::string SerializeGraphDataCompressed(const GraphData& data) {
  ...
  uint32_t node_count = data.nodes.size();
  char node_count_buf[sizeof(node_count)];
  LittleEndian::Store32(node_count_buf, node_count);
  binary_data.append(node_count_buf, sizeof(node_count));
  ...
  for (const auto& node : data.nodes) {
    ...
    LittleEndian::Store64(buffer, id);
    binary_data.append(buffer, sizeof(id));
    LittleEndian::Store32(buffer, absl::bit_cast<uint32_t>(x));
    binary_data.append(buffer, sizeof(x));
    LittleEndian::Store32(buffer, absl::bit_cast<uint32_t>(y));
    binary_data.append(buffer, sizeof(y));
    LittleEndian::Store32(buffer, absl::bit_cast<uint32_t>(diff_score));
    binary_data.append(buffer, sizeof(diff_score));
    LittleEndian::Store64(buffer, anchor_id);
    binary_data.append(buffer, sizeof(anchor_id));
    LittleEndian::Store16(buffer, key_len);
    binary_data.append(buffer, sizeof(key_len));
    binary_data.append(node.key.data(), key_len);
  }

  uint32_t edge_count = data.edges.size();
  char edge_count_buf[sizeof(edge_count)];
  LittleEndian::Store32(edge_count_buf, edge_count);
  binary_data.append(edge_count_buf, sizeof(edge_count));
  ...
  for (const auto& edge : data.edges) {
    ...
    LittleEndian::Store64(buffer, supplier_id);
    binary_data.append(buffer, sizeof(supplier_id));
    LittleEndian::Store64(buffer, consumer_id);
    binary_data.append(buffer, sizeof(consumer_id));
  }
...
std::string GenerateBackgroundStyles(
  ...
  std::set<std::string> unique_colors;
  // NOLINTNEXTLINE
...
struct TokenAnnotationMapping {
  ...
  std::map<size_t, std::vector<const TensorAnnotation*>> span_starts;
  std::map<size_t, std::vector<const TensorAnnotation*>> span_ends;
...
TokenAnnotationMapping GetTokenAnnotationMapping(
  ...
  static constexpr auto kComputationAttributes =
      gtl::fixed_flat_set_of<absl::string_view>(
          {"to_apply", "condition", "body", "select", "scatter",
  ...
  auto get_id = [](absl::string_view s, const char* prefix) {
    if (!s.empty() && (s[0] == '%' || s[0] == '@')) {
      return absl::StrCat(prefix, s.substr(1));
    }
  ...
  for (size_t i = 0; i < tokens.size(); ++i) {
    ...
    if (tok.kind == TokKind::kName &&
        kComputationAttributes.contains(tok.value)) {
      size_t j = SkipSpaces(tokens, i + 1);
      if (j < tokens.size() && tokens[j].value == "=") {
        size_t k = SkipSpaces(tokens, j + 1);
        if (k < tokens.size()) {
  ...
  for (size_t i = 0; i < tokens.size(); ++i) {
    ...
    if (mapping.token_to_annotation.count(i)) {
      ...
      if (css_class[0] != '\0' || !anchor_attr.empty() ||
        ...
        parts += HtmlEscape(tokens[i].value);
      }
    }
...
std::string ConvertHloToHtml(
  ...
  for (const FileToc* p = hlo_dump_assets_create(); p->name != nullptr; ++p) {
